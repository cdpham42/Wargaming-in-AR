...
namespace HoloToolkit.Sharing
{
    public class CustomMessages : Singleton<CustomMessages>
    {
        ...
        public enum TestMessageID : byte
        {
            ...
            HologramTransform = MessageID.UserMessageIDStart,
            Max
        }
        ...
        void InitializeMessageHandlers()
        {
            ...
            // Listen to messages from RemoteHologramManager? Redundant with above message listening declaration
            for (byte index = (byte)TestMessageID.HologramTransform; index < (byte)TestMessageID.Max; index++)
            {
                if (MessageHandlers.ContainsKey((TestMessageID)index) == false)
                {
                    MessageHandlers.Add((TestMessageID)index, null);
        ...
        public void SendHologramTransform(Vector3 position, Quaternion rotation, int objID)
        {
            // If we are connected to a session, broadcast our hologram info
            if (serverConnection != null && serverConnection.IsConnected())
            {
                Debug.Log("Sending Hologram transform");
                // Create an outgoing network message to contain all the info we want to send
                NetworkOutMessage msg = CreateMessage((byte)TestMessageID.HologramTransform);
                msg.Write(objID);
